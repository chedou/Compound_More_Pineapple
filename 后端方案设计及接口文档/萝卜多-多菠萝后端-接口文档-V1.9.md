...
| url            | URL地址  | String | 必填     |      |
| menuld         | 菜单ID   | long   | 必填     |      |
| parentModuleID | 父模块ID | long   | 必填     |      |
| moduleCategory | 模块类别 | String | 必填     |      |
| createBy       | 创建人   | String | 必填     |      |
| gmtCreate      | 创建时间 | Long   | 必填     |      |
...
| id             | 模块id   | id     | 必填     |      |
| name           | 模块名   | String | 非必填   |      |
| url            | URL地址  | String | 非必填   |      |
| menuld         | 模块ID   | id     | 非必填   |      |
| parentModuleID | 父模块ID | long   | 非必填   |      |
| moduleCategory | 模块类别 | String | 非必填   |      |
| createBy       | 创建人   | String | 非必填   |      |
| gmtCreate      | 创建时间 | Long   | 非必填   |      |
...
请求地址： DELETE /a/manage/module/{one}
...
## 修改记录 20181015（黄）
1. banner_show 表中 industry_type 注释更改为 ：行业类型。

## 修改记录 20181016 (黄)
1.删除 in_new in_edit in_delete 这三个字段。